...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'RouteToll.id'
        db.delete_column('lok_routetoll', 'id')

        # Adding field 'RouteToll.routeoption_ptr'
        db.add_column('lok_routetoll', 'routeoption_ptr',
                      self.gf('django.db.models.fields.related.OneToOneField')(default=1, to=orm['lok.RouteOption'], unique=True, primary_key=True),
                      keep_default=False)


    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'RouteToll.id'
        raise RuntimeError("Cannot reverse this migration. 'RouteToll.id' and its values cannot be restored.")
        # Deleting field 'RouteToll.routeoption_ptr'
        db.delete_column('lok_routetoll', 'routeoption_ptr_id')
